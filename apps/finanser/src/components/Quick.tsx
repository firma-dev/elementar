...
export function Quick({ options, onAdd, onCategory, onDrop }: Props): JSX.Element {
  ...
  return (
    <div class="f-quick">
      ...
      {/* Формат показывает сам пример в поле — «кофе 250» объясняет всё, кроме
          прихода. Про него и сказано, одной короткой строкой: подсказка,
          повторяющая пример, занимает место и ничего не добавляет. */}
      {text === '' && added === null ? (
        <p class="f-quick__hint">«+» в начале — приход</p>
      ) : null}

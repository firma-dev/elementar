...
const NOISE = folded([
  ...
  'НОМЕРУ',
  'ТЕЛЕФОНА',
  // «Зарплата (аванс) от Рога и Копыта» и «Зарплата от Рога и Копыта» — один
  // работодатель и один источник дохода. Аванс это часть той же зарплаты, а не
  // вторая работа, и двумя строками в списке он врал про их число.
  'АВАНС',
])
...
/**
 * Идентификатор операции: длинная мешанина букв и цифр вроде
 * «A1B2C3D4X5E6F7G8». Он свой у каждой операции, и в ключе получателя ему не
 * место — иначе двадцать шесть переводов одному человеку становятся двадцатью
 * шестью разными «людьми».
 */
const TRANSACTION_ID = /^(?=.*\d)[A-Z0-9]{12,}$/
...
function cleanWords(description: string, translit: boolean, fallback?: string): string {
  ...
  folded.forEach((word, i) => {
    ...
    if (TRANSACTION_ID.test(word)) return
  ...
  // Ничего не осталось, но номер телефона в описании был. Тогда он и есть имя:
  // английская выгрузка называет получателя перевода только номером, и без
  // этой ветки все переводы всем людям сливались бы в одного безымянного.
  // Короткие числа сюда не попадают — это номера точек, а не телефоны.
  const phone = folded.find((word) => /^\d{10,}$/.test(word))

...
export function demoCsv(endsAt = new Date().toISOString().slice(0, 10)): string {
  ...
  for (let back = 11; back >= 0; back -= 1) {
    ...
    // Переводы людям. Без них раздел «Кому вы переводите» в демо пуст, а это
    // самая крупная статья у всех, кто платит за аренду переводом, — и как раз
    // та, которую выписка не объясняет: у банка тут только имя получателя.
    // Трое разных по смыслу: аренда каждый месяц одному и тому же человеку,
    // деньги жене на продукты дважды в месяц и разовое «скинуться» приятелю.
    push(
      at(back, 3),
      -6_500_000,
      'Перевод на номер 0079990000001. Получатель: Марина Игоревна К. Осуществлен через СБП.',
      11,
    )
    push(
      at(back, 8),
      -between(700_000, 1_400_000),
      'Перевод на номер 0079990000002. Получатель: Анна Сергеевна В. Осуществлен через СБП.',
      19,
    )
    push(
      at(back, 21),
      -between(500_000, 1_200_000),
      'Перевод на номер 0079990000002. Получатель: Анна Сергеевна В. Осуществлен через СБП.',
      20,
    )
    if (back % 3 === 1) {
      push(
        at(back, 15),
        -between(80_000, 350_000),
        'Перевод на номер 0079990000003. Получатель: Дмитрий Павлович Л. Осуществлен через СБП.',
        21,
      )
    }

...
export function sniffNotCsv(bytes: Uint8Array): string | null {
  ...
  // PK.. — zip, а значит xlsx, ods или просто архив
  if (starts([0x50, 0x4b, 0x03, 0x04]) || starts([0x50, 0x4b, 0x05, 0x06])) {
    return (
      'Это таблица Excel или архив. Финансер читает CSV: при выгрузке из банка ' +
      'выберите формат CSV.'
    )
  }
  // Старый .xls
  if (starts([0xd0, 0xcf, 0x11, 0xe0])) {
    return 'Это старый файл Excel (.xls). Финансер читает CSV.'
  }
...
/**
 * Декодирование байтов. Т-Банк отдаёт windows-1251, но выгрузка, прошедшая через

...
describe('ждём по ритму, а не по числу месяца', () => {
  const приход = (date: string, amount: number) => ({
    id: date,
    date,
    time: null,
    amount,
    description: ' Зарплата от ООО РОГА',
    mcc: null,
    bankCategory: null,
    account: 'карта',
    currency: null,
    category: 'Доход' as const,
    source: 'rule' as const,
  })

  it('зарплата дважды в месяц ждётся через две недели, а не в середине', () => {
    // Платят десятого и двадцать пятого. «Обычное число» такого источника —
    // двадцать четвёртое: середина между двумя настоящими датами, день, когда
    // не приходит ничего. От неё считалось «сколько можно тратить в день», и
    // выходило двадцать четыре дня вместо девяти.
    const list = [
      приход('2026-06-10', 12000000),
      приход('2026-06-25', 7000000),
      приход('2026-07-10', 12000000),
      приход('2026-07-25', 7000000),
      приход('2026-08-10', 12000000),
      приход('2026-08-25', 7000000),
    ]
    const next = nextArrival(byIncomeSource(list, '2026-08-31'), '2026-08-31')
    expect(next?.date).toBe('2026-09-09')
  })

  it('ежемесячный приход по-прежнему ждётся тем же числом', () => {
    const list = [
      приход('2026-06-05', 10000000),
      приход('2026-07-05', 10000000),
      приход('2026-08-05', 10000000),
    ]
    const next = nextArrival(byIncomeSource(list, '2026-08-20'), '2026-08-20')
    expect(next?.date).toBe('2026-09-05')
  })
})

...
describe('вид операции сильнее словаря', () => {
  ...
  it('узнаёт снятие, кэшбэк, комиссию и копилку', () => {
    ...
    // «Прочее», а не «Кредиты»: «Кредиты» — план `move`, и плата за
    // обслуживание вместе с комиссиями выпадала из трат целиком, а «Уходит
    // само» не могло показать ежемесячную плату банка.
    expect(categorize(tx('Плата за обслуживание'), {}).category).toBe('Прочее')

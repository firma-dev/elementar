...
describe('чей это банк и сколько счетов', () => {
  const шапка =
    'Дата операции;Выполнено банком;Номер документа;Поступления;Расходы;Валюта;' +
    'Детали операции (назначение платежа);Номер карты'
  ...
  const файл = [
    шапка,
    '"17.08.2026 11:00";"17.08.2026";"1";"";"110,00";"RUB";" Оплата покупки по карте. CARD **3523 17AUG RUB 110.00 LAVKA";"**3523"',
    // У операции по СБП банк не проставляет номер карты — колонка пустая.
    '"16.08.2026 22:48";"17.08.2026";"2";"";"240,00";"RUR";" Оплата покупки 240.00 RUB METHOD. Осуществлен через СБП.";""',
  ].join('\n')

  it('одна карта на файл — один счёт, а не два', async () => {
    const { parseStatementText } = await import('../src/statement.js')
    const result = parseStatementText(файл, 'account_statement_25.06.26.csv')
    // Строки без номера карты уезжали на отдельный счёт с именем файла, и в
    // переключателе появлялся призрачный «Счёт 1» рядом с «Карта ·3523» — при
    // том что карта одна и деньги те же.
    expect(result.accounts).toHaveLength(1)
    expect(result.transactions).toHaveLength(2)
    expect(new Set(result.transactions.map((t) => t.account)).size).toBe(1)
    // Идентификаторы после переноса остаются разными.
    expect(new Set(result.transactions.map((t) => t.id)).size).toBe(2)
  })

  it('банк узнаётся по подписи выгрузки', async () => {
    const { parseStatementText } = await import('../src/statement.js')
    const result = parseStatementText(файл, 'выписка.csv')
    expect(result.bank?.name).toBe('Райффайзен Банк')
    // Догадка называет, на чём основана: подписать чужие деньги наугад хуже,
    // чем не подписать.
    expect(result.bank?.why).toContain('ВЫПОЛНЕНО БАНКОМ')
  })

  it('незнакомая выгрузка не получает выдуманного имени', async () => {
    const { parseStatementText } = await import('../src/statement.js')
    const чужая = ['Дата;Сумма;Описание', '"05.05.2026";"-1 200,50";"Кофе"'].join('\n')
    expect(parseStatementText(чужая, 'другой.csv').bank).toBeNull()
  })

  it('несколько карт в файле остаются несколькими счетами', async () => {
    const { parseStatementText } = await import('../src/statement.js')
    const две = [
      шапка,
      '"17.08.2026 11:00";"17.08.2026";"1";"";"110,00";"RUB";" Покупка";"**3523"',
      '"17.08.2026 12:00";"17.08.2026";"2";"";"120,00";"RUB";" Покупка";"**7788"',
      '"17.08.2026 13:00";"17.08.2026";"3";"";"130,00";"RUB";" Покупка по СБП";""',
    ].join('\n')
    // Догадываться не о чем: какой из двух карт принадлежит операция без
    // номера, файл не говорит.
...
describe('выписка по карте и выписка по счёту — разные выгрузки', () => {
  const карта = [
    ...
    '"03.09.2026 10:23";"В обработке";"ZR1";"-270,00";"RUB";"-270,00";"RUB";" Дагестанская Лавка"',
    '"21.08.2026 19:31";"21.08.2026";"ZP1";"5 000,00";"RUB";"5 000,00";"RUB";" ATM 8593 RAIFFEISEN"',
  ].join('\n')

  const счёт = [
    'Дата операции;Выполнено банком;Номер документа;Поступления;Расходы;Валюта;Детали операции (назначение платежа);Номер карты',
    '"03.09.2026 10:23";"03.09.2026";"1";"";"270,00";"RUB";" Дагестанская Лавка";"**3523"',
    '"10.08.2026 00:00";"10.08.2026";"2";"125 590,52";"";"RUR";" Зарплата от КАПИТАЛ ГРУП";""',
  ].join('\n')

  it('вид узнаётся по колонкам, а не по имени файла', async () => {
    const { parseStatementText } = await import('../src/statement.js')
    // У выписки по карте нет ни колонки с номером карты (карта одна), ни
    // раздельных колонок прихода и расхода: сумма одна, со знаком.
    expect(parseStatementText(карта, 'что-то.csv').kind).toBe('card')
    expect(parseStatementText(счёт, 'что-то.csv').kind).toBe('account')
  })

  it('в выписке по карте приход почти пуст, и это правда, а не поломка', async () => {
    const { parseStatementText } = await import('../src/statement.js')
    const r = parseStatementText(карта, 'card.csv')
    expect(r.transactions).toHaveLength(2)
    // Единственный приход — внесение наличных в банкомате: зарплата и переводы
    // по карте не проходят, их видно только в выписке по счёту.
    expect(r.transactions.filter((t) => t.amount > 0)).toHaveLength(1)
  })
})

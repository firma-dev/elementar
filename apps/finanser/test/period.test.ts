...
describe('всё загруженное', () => {
  it('«всё» не обрезает ничего снизу', () => {
    // Шесть кнопок закрывают ежедневные вопросы, но данные старше года иначе
    // стали бы недоступны молча — а это хуже лишней ссылки (Д-026).
    const all = bounds('all', '2026-08-24')
    expect(all.to).toBe('2026-08-24')
    expect(all.from < '1900-01-01').toBe(true)
  })

  it('«всё» — не дневной режим', () => {
    expect(isDaily('all')).toBe(false)
  })
})
